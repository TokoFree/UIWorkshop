//
//  StackLayoutVCInteractive.swift
//  TextureWorkshop
//
...
internal class StackLayoutVCInteractive: ASDKViewController<ASDisplayNode> {
    
    // MARK: UI Elements
    
    private let childVC = StackLayoutVC()
    ...
    private let spacingText: ASTextNode2 = {
        let node = ASTextNode2()
        node.attributedText = NSAttributedString(string: "spacing")
        node.style.width = ASDimensionMake(60)
        return node
    }()
    private let spacingOptions = StackSpacingOptionsNode()
    
    private let justifyText: ASTextNode2 = {
        let node = ASTextNode2()
        node.attributedText = NSAttributedString(string: "justify")
        node.style.width = ASDimensionMake(60)
        return node
    }()
    private let justifyOptions = StackJustifyOptionsNode()
    
    private let alignText: ASTextNode2 = {
        let node = ASTextNode2()
        node.attributedText = NSAttributedString(string: "align")
        node.style.width = ASDimensionMake(60)
        return node
    }()
    private let alignOptions = StackAlignOptionsNode()
    ...
    internal override init() {
        ...
        node.layoutSpecBlock = { [weak self] _, size -> ASLayoutSpec in
            ...
            let spacingStack = ASStackLayoutSpec(direction: .horizontal, spacing: 8, justifyContent: .start, alignItems: .center, children: [self.spacingText, self.spacingOptions])
            let wrapper = ASWrapperLayoutSpec(layoutElement: self.justifyOptions)
            wrapper.style.flexShrink = 1
            let justifyStack = ASStackLayoutSpec(direction: .horizontal, spacing: 8, justifyContent: .start, alignItems: .center, children: [self.justifyText, wrapper])
            let alignStack = ASStackLayoutSpec(direction: .horizontal, spacing: 8, justifyContent: .start, alignItems: .center, children: [self.alignText, self.alignOptions])
            
            let optionStack = ASStackLayoutSpec(direction: .vertical, spacing: 8, justifyContent: .start, alignItems: .start, children: [directionStack, spacingStack, justifyStack, alignStack])
    ...
    private func setupOptions() {
        ...
        spacingOptions.onPicked = { [weak self] val in
            self?.childVC.setSpacing(val)
        }
        spacingOptions.tapButton0()
        
        justifyOptions.onPicked = { [weak self] val in
            self?.childVC.setJustify(val)
        }
        justifyOptions.tapButtonStart()
        
        alignOptions.onPicked = { [weak self] val in
            self?.childVC.setAlign(val)
        }
        alignOptions.tapButtonStart()
    }
